...
parser.add_argument('--dataroot', required=True,
                    help='dataroot contains: train, valid, vocab.slot_tag, vocab.act_tag, vocab.unali, vocab.act')
...
parser.add_argument('--testing', action='store_true',
                    help='Online test: only test your model (default is training && testing)')
...
parser.add_argument('--add_pred_rule', action='store_true',
                    help='Whether to consider BIO rule when predicting tags (default is not)')
...
parser.add_argument('--decoder_tied', action='store_true',
                    help='To tie the output layer and input embedding in decoder')
...
parser.add_argument('--st_weight', type=float, default=1,
                    help='loss weight for slot tagging task, ranging from 1 to the max.')
...
intent_tag_vocab_dir = dataroot + '/lab_unali'
...
if not opt.testing:
    ...
    intent_tag_to_idx, idx_to_intent_tag = data_reader.read_vocab_file(intent_tag_vocab_dir)
    word_to_idx, idx_to_word = data_reader.read_vocab_from_data_file(train_data_dir, vocab_config=vocab_config)
else:
    slot_tag_to_idx, idx_to_slot_tag = data_reader.read_vocab_file(opt.read_vocab + '.slot_tag', bos_eos=False,
                                                                   no_pad=True, no_unk=True)
    word_to_idx, idx_to_word = data_reader.read_vocab_file(opt.read_vocab + '.in', bos_eos=False, no_pad=True,
                                                           no_unk=True)
...
if not opt.testing:
    train_feats, train_slot_tags, train_intent_tags = data_reader.read_seqtag_data_with_unali_act(train_data_dir,
                                                                                                  word_to_idx,
                                                                                                  slot_tag_to_idx,
                                                                                                  intent_tag_to_idx)
    valid_feats, valid_slot_tags, valid_intent_tags = data_reader.read_seqtag_data_with_unali_act(valid_data_dir,
                                                                                                  word_to_idx,
                                                                                                  slot_tag_to_idx,
                                                                                                  intent_tag_to_idx,
                                                                                                  keep_order=opt.testing)
    test_feats, test_slot_tags, test_intent_tags = data_reader.read_seqtag_data_with_unali_act(test_data_dir,
                                                                                               word_to_idx,
                                                                                               slot_tag_to_idx,
                                                                                               intent_tag_to_idx,
                                                                                               keep_order=opt.testing)
else:
    lab_test_feats, lab_test_slot_tags, lab_test_intent_tags = data_reader.read_seqtag_data_with_unali_act(
        lab_test_data_dir, word_to_idx, slot_tag_to_idx, intent_tag_to_idx, keep_order=opt.testing, raw_word=True)
    rec_test_feats, rec_test_slot_tags, rec_test_intent_tags = data_reader.read_seqtag_data_with_unali_act(
        rec_test_data_dir, word_to_idx, slot_tag_to_idx, intent_tag_to_idx, keep_order=opt.testing, raw_word=True)

if opt.task == 'slot_tagger':
    import models.slot_tagger as slot_tagger

    model_tag = slot_tagger.LSTMTagger(opt.emb_size, opt.hidden_size, len(word_to_idx), len(slot_tag_to_idx),
                                       word_to_idx['<pad>'], bidirectional=opt.bidirectional, num_layers=opt.num_layers,
    ...
    model_tag = slot_tagger_with_focus.LSTMTagger_focus(opt.emb_size, opt.hidden_size, len(word_to_idx),
                                                        len(slot_tag_to_idx), len(intent_tag_to_idx),
                                                        word_to_idx['<pad>'], slot_tag_to_idx['<pad>'],
                                                        bidirectional=opt.bidirectional, num_layers=opt.num_layers,
                                                        dropout=opt.dropout, device=opt.device)
elif opt.task == 'slot_tagger_with_crf':
    import models.slot_tagger_crf as slot_tagger_crf

    model_tag = slot_tagger_crf.LSTMTagger_CRF(opt.emb_size, opt.hidden_size, len(word_to_idx), len(slot_tag_to_idx),
                                               word_to_idx['<pad>'], bidirectional=opt.bidirectional,
                                               num_layers=opt.num_layers, dropout=opt.dropout, device=opt.device)
...
intent_loss_function = nn.BCELoss(size_average=False)
...
def decode(data_feats, data_slot_tags, data_intent_tags, output_path):
    data_index = np.arange(len(data_feats))
    losses = []
    TP, FP, FN, TN = 0.0, 0.0, 0.0, 0.0
    ITP, IFP, IFN, ITN = 0.0, 0.0, 0.0, 0.0
    with open(output_path, 'w') as f:
        for j in range(0, len(data_index), opt.test_batchSize):
            if opt.testing:
                inputs, slot_tags, intent_tags, lens, line_nums, raw_words = data_reader.get_minibatch_with_unali_act(
                    data_feats, data_slot_tags, data_intent_tags, word_to_idx, slot_tag_to_idx, data_index, j,
                    opt.test_batchSize, add_start_end=opt.bos_eos, keep_order=opt.testing, raw_word=True,
                    enc_dec_focus=opt.enc_dec, device=opt.device)
            else:
                inputs, slot_tags, intent_tags, lens = data_reader.get_minibatch_with_unali_act(data_feats,
                                                                                                data_slot_tags,
                                                                                                data_intent_tags,
                                                                                                word_to_idx,
                                                                                                slot_tag_to_idx,
            ...
            if opt.enc_dec:
                ...
                _, intent_socre, _ = model_tag(inputs, slot_tags[:, :-1], lens)
                gate_value = 0.8 * torch.ones(intent_tags.shape, dtype=torch.float, device=opt.device)
                intent_tags = intent_tags > gate_value
                pred_intent = intent_socre > gate_value

                ITP += float(torch.sum(intent_tags & (intent_tags == pred_intent)))
                ITN += float(torch.sum((1 - intent_tags) & (intent_tags == pred_intent)))
                IFP += float(torch.sum(pred_intent & (intent_tags != pred_intent)))
                IFN += float(torch.sum((1 - pred_intent) & (intent_tags != pred_intent)))

                opt.greed_decoding = True  # True, False
                if opt.greed_decoding:
                    slot_tag_scores_1best, pred_slot_tag_1best, h_t_c_t = model_tag.decode_greed(inputs,
                                                                                                 slot_tags[:, 0:1],
                                                                                                 lens)
                    slot_tag_loss = slot_tag_loss_function(
                        slot_tag_scores_1best.contiguous().view(-1, len(slot_tag_to_idx)),
                        slot_tags[:, 1:].contiguous().view(-1))
                    pred_slot_tag_1best = pred_slot_tag_1best.cpu().numpy()
                else:
                    beam_size = 2
                    beam_tag_scores_1best, pred_slot_tag_1best, _ = model_tag.decode_beam_search(inputs, lens,
                                                                                                 beam_size,
                                                                                                 slot_tag_to_idx)
                ...
                slot_tag_loss = slot_tag_loss_function(slot_tag_scores.contiguous().view(-1, len(slot_tag_to_idx)),
                                                       slot_tags.view(-1))
            ...
            for idx, pred_line in enumerate(pred_slot_tag_1best):
                ...
                intent_idxs = list(torch.nonzero(pred_intent[idx]))
                if not intent_idxs:
                    intent_idxs = [0]

                intent_seq = [idx_to_intent_tag[int(x)] for x in intent_idxs]
                if opt.testing:
                    f.write(str(line_nums[idx]) + ' : ' + ' '.join(word_tag_line) + '\n')
                else:
                    f.write(' '.join(word_tag_line) + ' <=> '+ (';'.join(intent_seq)) +'\n')
    ...
    if ITP == 0:
        ...
        i_p, i_r, i_f = 100 * ITP / (ITP + IFP), 100 * ITP / (ITP + IFN), 100 * 2 * ITP / (2 * ITP + IFN + IFP)

    mean_losses = np.mean(losses, axis=0)

    return mean_losses, p, r, f, i_p, i_r, i_f
...
if not opt.testing:
    ...
    for i in range(opt.max_epoch):
        ...
        for j in range(0, len(train_data_index), opt.batchSize):
            inputs, slot_tags, intent_tags, lens = data_reader.get_minibatch_with_unali_act(train_feats['data'],
                                                                                            train_slot_tags['data'],
                                                                                            train_intent_tags['data'],
                                                                                            word_to_idx,
                                                                                            slot_tag_to_idx,
                                                                                            train_data_index, j,
                                                                                            opt.batchSize,
            ...
            # slot tag
            if opt.enc_dec:
                slot_tag_scores, intent_socre, h_t_c_t = model_tag(inputs, slot_tags[:, :-1], lens)
                slot_tag_loss = slot_tag_loss_function(slot_tag_scores.contiguous().view(-1, len(slot_tag_to_idx)),
                                                       slot_tags[:, 1:].contiguous().view(-1))
                intent_loss = intent_loss_function(intent_socre, intent_tags)
            elif opt.crf:
                max_len = max(lens)
                masks = [([1] * l) + ([0] * (max_len - l)) for l in lens]
                masks = torch.tensor(masks, dtype=torch.uint8, device=opt.device)
                ...
                slot_tag_loss = slot_tag_loss_function(slot_tag_scores.contiguous().view(-1, len(slot_tag_to_idx)),
                                                       slot_tags.view(-1))
            slot_losses.append(slot_tag_loss.item() / sum(lens))
            intent_losses.append((intent_loss.item()))

            total_loss = slot_tag_loss / sum(lens) + intent_loss
            total_loss.backward()
        ...
        mean_slot_loss = np.mean(slot_losses, axis=0)
        mean_intent_loss = np.mean(intent_losses, axis=0)
        logger.info(
            'Training:\tEpoch : %d\tTime : %.4fs\tLoss of slot tag : %.5f\tLoss of intent: %.5f' % (
            i, time.time() - start_time, mean_slot_loss, mean_intent_loss))
        gc.collect()
        ...
        loss_val, p_val, r_val, f_val, ip_val, ir_val, if_val = decode(valid_feats['data'], valid_slot_tags['data'],
                                                                       valid_intent_tags['data'],
                                                                       os.path.join(exp_path, 'valid.iter' + str(i)))
        logger.info('Validation:\tEpoch : %d\tTime : %.4fs\tLoss : %.5f\tFscore : %.5f\tIntent Fscore : %.5f' % (
            i, time.time() - start_time, loss_val, f_val, if_val))
        start_time = time.time()
        loss_te, p_te, r_te, f_te, ip_te, ir_te, if_te = decode(test_feats['data'], test_slot_tags['data'],
                                                                valid_intent_tags['data'],
                                                                os.path.join(exp_path, 'test.iter' + str(i)))
        logger.info('Evaluation:\tEpoch : %d\tTime : %.4fs\tLoss : %.5f\tFscore : %.5f\tIntent Fscore : %.5f' % (
            i, time.time() - start_time, loss_te, f_te, if_te))

        if best_f1 < f_val:
            model_tag.save_model(os.path.join(exp_path, opt.save_model + '.tag'))
            best_f1 = f_val
            logger.info('NEW BEST:\tEpoch : %d\tbest valid F1 : %.5f\tbest valid Intent F1 : %.5f\ttest F1 : %.5f \ttest Intent Fscore : %.5f' % (i, f_val, if_val, f_te, if_te))
            best_result['iter'] = i
            best_result['vf1'], best_result['vce'] = f_val, loss_val
            best_result['tf1'], best_result['tce'] = f_te, loss_te
    logger.info('BEST RESULT: \tEpoch : %d\tbest valid (Loss: %.5f F1 : %.5f)\tbest test (Loss: %.5f F1 : %.5f) ' % (
        best_result['iter'], best_result['vce'], best_result['vf1'], best_result['tce'], best_result['tf1']))
else:
    logger.info("Online testing starts at %s" % (time.asctime(time.localtime(time.time()))))
    model_tag.eval()
    start_time = time.time()
    loss_te, p_te, r_te, f_te, ip_te, ir_te, if_te = decode(lab_test_feats['data'], lab_test_slot_tags['data'],
                                                            lab_test_intent_tags['data'].
                                                            os.path.join(exp_path, 'test.lab'))
    logger.info('Evaluation:\tEpoch : %d\tTime : %.4fs\tLoss : %.5f\tFscore : %.5f' % (
        0, time.time() - start_time, loss_te, f_te))
    start_time = time.time()
    loss_te, p_te, r_te, f_te, ip_te, ir_te, if_te = decode(rec_test_feats['data'], rec_test_slot_tags['data'],
                                                            rec_test_intent_tags['data'].
                                                            os.path.join(exp_path, 'test.rec'))
    logger.info('Evaluation:\tEpoch : %d\tTime : %.4fs\tLoss : %.5f\tFscore : %.5f' % (
        0, time.time() - start_time, loss_te, f_te))
